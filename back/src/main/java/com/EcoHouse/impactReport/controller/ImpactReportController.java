...
//    @PreAuthorize("hasRole('CUSTOMER') or hasRole('ADMIN')")
    public ResponseEntity<ImpactReportResponseDto> generateReport(
        ...
        // 1. Validación automática (@Valid) -> 400 si inválido
        log.info("Received request to generate impact report for customer: {}", request.getCustomerId());

        // 2. Llamada al service que realiza el trabajo:
        //    - validar existencia/periodo/conflictos
        //    - calcular métricas (CO2, EcoPoints, etc.)
        //    - persistir el informe
        ImpactReportResponseDto report = impactReportService.generateReport(request);

        // 3. Responder con 201 Created y el DTO resultante
